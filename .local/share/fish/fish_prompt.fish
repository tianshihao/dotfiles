# move the ~/.local/share/omf/themes/coffeeandcode/functions
# name: CoffeeAndCode
# Theme colors
...
set __fish_git_prompt_color bryellow
...
function fish_prompt --description 'Write out the prompt'
  ...
  echo -s "$__fish_prompt_user" "$USER" @ "$__fish_prompt_host" "$__fish_prompt_hostname" "$__fish_prompt_normal"

  echo -s "$__fish_prompt_cwd" "$path_emoji" ' ' (prompt_pwd) "$__fish_prompt_normal"

  set -l git_prompt (__fish_git_prompt)
  if test -n "$git_prompt"
    set -l git_color (set_color $__fish_git_prompt_color)
    echo -s "$git_color" "$git_emoji" "$__fish_prompt_normal" "$git_prompt" "$__fish_prompt_normal"
  end
